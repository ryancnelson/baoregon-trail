#!/bin/bash
# tools/run_zork1boot_qemu.sh -- build and run main_qemu_zork1boot.c
...
#
# Usage:
#   tools/run_zork1boot_qemu.sh                  # build + run headless (-display none)
#   tools/run_zork1boot_qemu.sh --cocoa          # build + run with a real window (macOS)
#   tools/run_zork1boot_qemu.sh --build-only      # just build the ELF, don't run
set -euo pipefail
...
SOURCES=(
    ...
    "$REPO_ROOT/src/main_qemu_zork1boot.c"
...
for src in "${SOURCES[@]}"; do
    base="$(basename "$src")"
    obj="$BUILD_DIR/zork1boot_${base%.*}.o"
    "$CC" $CFLAGS -c -o "$obj" "$src"
    OBJECTS+=("$obj")
done

ELF="$BUILD_DIR/baoregon-zork1boot-qemu.elf"
