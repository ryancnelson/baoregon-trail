#!/bin/bash
# tools/run_dos33boot_qemu.sh -- build and run main_qemu_dos33boot.c
# under real QEMU with a live ramfb display, matching the same
# cross-compile pattern as tools/run_loderunner_qemu.sh /
# tools/run_ramfb_qemu_test.sh.
#
# Usage:
#   tools/run_dos33boot_qemu.sh                  # build + run headless (-display none)
#   tools/run_dos33boot_qemu.sh --cocoa          # build + run with a real window (macOS)
#   tools/run_dos33boot_qemu.sh --build-only      # just build the ELF, don't run
set -euo pipefail
...
SOURCES=(
    ...
    "$REPO_ROOT/src/main_qemu_dos33boot.c"
...
for src in "${SOURCES[@]}"; do
    base="$(basename "$src")"
    obj="$BUILD_DIR/dos33boot_${base%.*}.o"
    "$CC" $CFLAGS -c -o "$obj" "$src"
    OBJECTS+=("$obj")
done

ELF="$BUILD_DIR/baoregon-dos33boot-qemu.elf"
