...
static void *rram_memcpy(void *dest, const void *src, size_t n) {
    uint8_t *d = (uint8_t *)dest;
    const uint8_t *s = (const uint8_t *)src;
    for (size_t i = 0; i < n; i++) d[i] = s[i];
    return dest;
}
static int rram_memcmp(const void *s1, const void *s2, size_t n) {
...
#define memcpy rram_memcpy
#define memcmp rram_memcmp
