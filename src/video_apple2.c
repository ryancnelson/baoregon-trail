...
/*
 * Apple II Hi-Res Graphics (280x192) 192-line interleaved memory lookup table.
 * Derived from formula: offset(y) = ((y % 8) * 0x0400) + (((y / 8) % 8) * 0x0080) + ((y / 64) * 0x0028)
 */
...
void hires_decode_scanline_color(int row, read6502_fn read_mem,
    ...
    hires_decode_scanline_color_page(row, 0, read_mem, out_colors);
}
