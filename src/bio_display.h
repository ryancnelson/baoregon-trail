 ...
 * mixed_mode is 0 (full-screen TEXT, not MIXED), this function renders
 * real character-ROM glyphs via text_apple2_render_frame() (see
 * text_apple2.h -- ported from the real, MAME-verified 342-0133-a.chr
 * Apple IIe character generator ROM, closing the gap this doc comment
 * used to describe as "no character ROM data exists yet"). When
 * mixed_mode is nonzero, TEXT/GRAPHICS is irrelevant (real Apple II
 * MIXED mode always shows graphics on top regardless of the
 * TEXT/GRAPHICS switch) -- this function proceeds with the normal
 ...
 * unlike the full-TEXT case where text_apple2_render_frame() runs
 * every frame instead).
 */
